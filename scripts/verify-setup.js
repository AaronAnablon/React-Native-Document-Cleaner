...
try {
  ...
  console.error('❌ Could not find package.json. Make sure you\'re in a React Native project root.');
  process.exit(1);
}

// Check if react-native-document-scanner-ai is installed
const isInstalled = packageJson.dependencies && packageJson.dependencies['react-native-document-scanner-ai'] ||
                   packageJson.devDependencies && packageJson.devDependencies['react-native-document-scanner-ai'];

if (!isInstalled) {
  console.error('❌ react-native-document-scanner-ai is not installed.');
  console.log('   Install it with: npm install react-native-document-scanner-ai');
  process.exit(1);
}
...
if (fs.existsSync(iosPath)) {
  ...
  if (fs.existsSync(podfilePath)) {
    ...
    console.log('⚠️  Podfile not found - you may need to run: npx react-native init --template react-native@latest');
  }
  
  // Check for Info.plist
  const infoPlistPath = path.join(iosPath, path.basename(process.cwd()), 'Info.plist');
...
const nodeModulesPath = path.join(process.cwd(), 'node_modules', 'react-native-document-scanner-ai');
...
console.log('2. For Android: Make sure permissions are set in AndroidManifest.xml');
